...
from ListGenerator import generate_list
import itertools
...
def save_features_as_json1(structured_list):
    ...
    try:
        ...
        with open('FeatureList.json', 'w') as file:
...
def save_features_as_json2(structured_data, proposal_id, feature_list_id, filename='FeatureListDatabase.json'):
...
def save_proposal(user_vision, final_vision, feature_list_id, proposal_id, filename='Proposals.json'):
    new_proposal = {
...
def main():
    ...
    try:
        with open('FeatureList.json', 'r') as file:
