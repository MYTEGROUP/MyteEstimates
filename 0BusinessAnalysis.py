...
def text_to_speech(input_text):
    ...
    timestamp = int(time.time())  # Get current time to use as a unique identifier
...
def submit_response():
    ...
    # Convert the next question to speech and get the unique audio file path
    audio_file_path = text_to_speech(next_question)

    # Extract directory and file name
    temp_dir, audio_filename = os.path.split(audio_file_path)

    # Return the next question and the path to the audio file (relative to the temporary directory)
...
def serve_audio(directory, filename):
    # Serve the file from the provided directory
